...
const selectData = (sortby,sort,page,limit,search) => {
    return Pool.query(`SELECT products.id,products.name,products.stock,products.price,category.name as category FROM products INNER JOIN category ON products.category_id = category.id WHERE products.name ILIKE '%${search}%' ORDER BY ${sortby} ${sort} limit ${limit} offset ${(page-1)*limit}`);
}
const selectDatabyId = (id) => {
    return Pool.query(`SELECT products.id,products.name,products.stock,products.price,category.name as category FROM products INNER JOIN category ON products.category_id = category.id WHERE products.id=${id}`);
}
...
const insertData = (data) => {
    ...
    return Pool.query(`INSERT INTO products(id,name,stock,price,category_id) VALUES(${id},'${name}',${stock},${price},${category_id})`);
}
